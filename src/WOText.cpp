...
WOText* WOText::New(const std::string& fontPath, float fontSize, std::string initial, std::string label) {
	WOText* wo = new WOText();
	wo->onCreate(fontPath, fontSize, initial, label);
	return wo;
}

WOText::WOText() : IFace(this), WOFTGLString() {}

void WOText::onCreate(const std::string& fontPath, float fontSize, std::string initial, std::string label) {
