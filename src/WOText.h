...
namespace Aftr {
    ...
    public:
        static WOText* New(const std::string& fontPath, float fontSize, std::string initial = "", std::string label = "");
        virtual ~WOText() {}
    ...
    protected:
        WOText();
        virtual void onCreate(const std::string& fontPath, float fontSize, std::string initial = "", std::string label = "");
